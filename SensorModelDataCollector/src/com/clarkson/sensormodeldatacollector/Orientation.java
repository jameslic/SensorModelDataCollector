 ...
 * Created by jlicata on 2/13/2016.
 */
...
public class Orientation implements SensorEventListener {
    ...
    private void performMagnetometerCalibrationPhase_1(SensorEvent event) {
        ...
        //Toast.makeText(sketchView.getContext(), "Magnetometer calibration Phase 1", Toast.LENGTH_SHORT).show();
    }
    ...
    private void checkCalibrationPhase2Termination(float epsilonX, float epsilonY) {
        ...
        if (calibrationXvalues.size() >= MIN_CALIBRATION_POINTS &&
            ...
            Toast.makeText(sketchView.getContext(), "Calibration complete!", Toast.LENGTH_LONG).show();

        }
        else {
                Toast.makeText(sketchView.getContext(), "#xValues = " + calibrationXvalues.size() + "#yValues = " + calibrationYvalues.size()+ "Magnetometer calibration: Phase 2, Slowly turn around with phone", Toast.LENGTH_LONG).show();

            if(calibrationXvalues.size() >= 20)
            {
                calibrationRangeValues = findCalibrationMinMaxValues(calibrationXvalues, calibrationYvalues);
                calibrationXvalues = null;
                calibrationYvalues = null;
